...
| disable_refresh_table_pks                 | Boolean |            | (Default: False) By default the connector refresh tables primary keys on each run. This can have a negative impact on performance. With `disable_refresh_table_pks` option you can turn off the refresh of primary keys is your schema does not changed. |
