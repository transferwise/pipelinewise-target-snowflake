...
class TestIntegration(unittest.TestCase):
    ...
    def setUp(self):
        ...
        if self.config["stage"]:
            snowflake.query(
                "DELETE FROM {}.columns WHERE TABLE_SCHEMA ilike '{}'".format(
                    snowflake.pipelinewise_schema, self.config["default_target_schema"]
                )
            )
    ...
    def assert_three_streams_are_into_snowflake(
    ...
    def test_loading_tables_with_client_side_encryption_and_wrong_master_key(self):
        ...
        self.config[
            "client_side_encryption_master_key"
        ] = "Wr0n6m45t3rKeY0123456789a0123456789a0123456="
    ...
    def test_loading_unicode_characters(self):
        ...
        table_unicode = snowflake.query(
            "SELECT * FROM {}.test_table_unicode ORDER BY C_INT".format(target_schema)
        )
    ...
    def test_column_name_change(self):
        ...
        previous_column_name = snowflake.query(
            ...
            """.format(
                self.config.get("dbname", "").upper(), target_schema.upper()
            )
        )[0]["COLUMN_NAME"]

        # Table one should have no changes
        self.assertEqual(table_one, [{"C_INT": 1, "C_PK": 1, "C_VARCHAR": "1"}])
    ...
    def test_information_schema_cache_create_and_update(self):
        ...
        self.assertEqual(
            ...
            [
                {
                    "TABLE_SCHEMA": target_schema.upper(),
    ...
    def test_information_schema_cache_outdated(self):
        ...
        """.format(
                snowflake.pipelinewise_schema, target_schema
            )
        )
        ...
        snowflake.query(
            "CREATE OR REPLACE TABLE {}.test_table_one (C_PK NUMBER, C_INT NUMBER, C_VARCHAR TEXT)".format(
                target_schema
            )
        )
