...
class DbSync:
    ...
    def cache_information_schema_columns(self, table_schemas=[], create_only=False):
        ...
        if not create_only and table_schemas:
            ...
            delete = delete + " WHERE LOWER(table_schema) IN ({})".format(
                ", ".join("'{}'".format(s).lower() for s in table_schemas)
            )
            ...
            insert = insert + " WHERE LOWER(table_schema) IN ({})".format(
                ", ".join("'{}'".format(s).lower() for s in table_schemas)
            )
    ...
    def create_table_query(self, is_temporary=False):
        ...
        primary_key = (
            ["PRIMARY KEY ({})".format(", ".join(primary_column_names(stream_schema_message)))]
    ...
    def grant_usage_on_schema(self, schema_name, grantee):
        ...
        logger.info(
            "Granting USAGE privilegue on '{}' schema to '{}'... {}".format(
    ...
    def create_schema_if_not_exists(self):
        ...
        if self.information_schema_columns:
            schema_rows = list(
                filter(lambda x: x["TABLE_SCHEMA"] == schema_name, self.information_schema_columns)
            )
    ...
    def get_table_columns(
        self, table_schemas=[], table_name=None, from_information_schema_cache_table=False
    ):
        ...
        if table_schemas or table_name:
            ...
            """.format(
                "information_schema"
                if not from_information_schema_cache_table
                else self.pipelinewise_schema
            )
            if table_schemas:
                sql = sql + " WHERE LOWER(c.table_schema) IN ({})".format(
                    ", ".join("'{}'".format(s).lower() for s in table_schemas)
                )
            elif table_name:
    ...
    def update_columns(self):
        ...
        if self.information_schema_columns:
            columns = list(
                filter(
                    ...
                    and x["TABLE_NAME"].lower() == table_name,
                    self.information_schema_columns,
                )
            )
        else:
            columns = self.get_table_columns(
                table_schemas=[self.schema_name], table_name=table_name
            )
        ...
        columns_to_replace = [
            ...
            != column_type(name, properties_schema).lower().split("(")[0]
            and
    ...
    def drop_column(self, column_name, stream):
        drop_column = "ALTER TABLE {} DROP COLUMN {}".format(
            self.table_name(stream, False), column_name
        )
