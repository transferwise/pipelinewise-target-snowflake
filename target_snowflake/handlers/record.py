...
import target_snowflake.handlers.utils as utils
from joblib import Parallel, delayed, parallel_backend


def transform_records(config, schema, record_messages, validator):
    """ Generator to transform a collection of record_messages from the same stream.
    """
    for record_message in record_messages:
        # Validate record
        # TODO: should we validate before or after transforming?
        if config.get('validate_records'):
            utils.validate_record(record_message, validator)

        # Add metadata values to record
        if config.get('add_metadata_columns') or config.get('hard_delete'):
            record_message = utils.add_metadata_values_to_record(record_message)

        # Truncate timestamps that are unsupported by Snowflake
        record_message = utils.adjust_timestamps_in_record(
            record_message, schema
        )
        yield record_message['record']


# pylint: disable=too-many-arguments
def flush_buckets(
    config, schema, validator, buckets, streams_to_flush,
    ...
    if records:
        ...
        if no_compression:
            with open(csv_fd, 'wb') as outfile:
                transform_and_write_record_to_file(
                    config, schema, validator,
                    records, outfile, record_to_csv_line_transformer
                )
        else:
            with open(csv_fd, 'wb') as outfile:
                with gzip.GzipFile(filename=csv_file, mode='wb', fileobj=outfile) as gzipfile:
                    transform_and_write_record_to_file(
                        config, schema, validator,
                        records, gzipfile, record_to_csv_line_transformer
                    )
...
def transform_and_write_record_to_file(
    config, schema, validator, records, outfile, record_to_csv_line_transformer
):
    for record in transform_records(config, schema, records, validator):
